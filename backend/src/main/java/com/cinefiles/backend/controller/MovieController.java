...
public class MovieController {
    ...
    // Inject the repository directly for quick local DB lookups
    @Autowired
    private com.cinefiles.backend.repository.MovieRepository movieRepository;

    // --- THE AUTOCOMPLETE SEARCH ENDPOINT ---
    @GetMapping("/autocomplete")
    public ResponseEntity<?> autocompleteMovies(@RequestParam String query) {
        try {
            List<Movie> matches = movieRepository.findByTitleContainingIgnoreCase(query);
